...
from common import build_model_frame, configure_matplotlib, encode_bclc, fit_rsf, km_estimate, load_raw_data, locate_stage_columns, split_xy, style_axis, style_panel_title
...
for label, color in [("Stage A", "#2C3E50"), ("Stage B", "#7F8C8D")]:
    subset = frame[frame["BCLC_Group"] == label]
...
style_panel_title(axes[0], "Traditional Clinical Staging (BCLC)")
style_axis(axes[0])
axes[0].legend(frameon=False)

stage_a = frame[frame["BCLC_Group"] == "Stage A"]
for label, color in [("Low risk", "#2E86C1"), ("High risk", "#C0392B")]:
    subset = stage_a[stage_a["RiskGroup"] == label]
    t, s, l, u = km_estimate(subset["Time"], subset["Event"])
    axes[1].step(t, s, where="post", lw=2.6, color=color, label=label)
    axes[1].fill_between(t, l, u, step="post", color=color, alpha=0.12)
style_panel_title(axes[1], "RSF Stratification Within BCLC Stage A")
style_axis(axes[1])
axes[1].legend(frameon=False)

fig.tight_layout()
fig.savefig(Path(__file__).with_name("BCLC_EarlyStage_RiskStratification.png"), bbox_inches="tight")
